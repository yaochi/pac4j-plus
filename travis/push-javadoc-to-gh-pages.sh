...
if [ "$invokeDoc" == true ]; then
  ...
  cp -R docs/pac4j-plus-documentation $HOME/docs-latest
...
if [[ "$invokeJavadoc" == true || "$invokeDoc" == true ]]; then
  ...
  git clone --single-branch --depth 3 --branch gh-pages --quiet https://${GH_TOKEN}@github.com/apereo/XieXianbin/pac4j-plus gh-pages > /dev/null
